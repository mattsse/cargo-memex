use super::*;
use crate::jpeg::decoder::parser::read_length;
use crate::jpeg::{AppMarkerConfig, APP0_DECIMAL};
...
impl<R: Read, W: Write> Decoder<R, W> {
    ...
    pub fn new(reader: R, marker_writer: W, config: AppMarkerConfig) -> Decoder<R, W> {
        Decoder {
            reader,
            app_marker: config,
            marker_writer,
    ...
    fn decode_internal(&mut self, stop_after_metadata: bool) -> Result<Vec<u8>> {
        ...
        loop {
            ...
            match marker {
                ...
                // Application data
                Marker::APP(app) => {
                    if app.saturating_add(APP0_DECIMAL) == self.app_marker.marker {
                        let length =
                            read_length(&mut self.reader, Marker::APP(self.app_marker.marker))?;
                        if length > self.app_marker.vendor.len() {
                            let mut buf = vec![0u8; self.app_marker.vendor.len()];
                            self.reader.read_exact(&mut buf)?;
                            if buf.as_slice() == self.app_marker.vendor.as_bytes() {
                                buf = vec![0u8; length - self.app_marker.vendor.len()];
                                self.reader.read_exact(&mut buf)?;
                                self.marker_writer.write_all(&buf)?;
                            }
                        }
                    } else {
